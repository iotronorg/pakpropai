...
def _header(s):
    ...
    return [
        Paragraph('PakProp AI', s['brand_title']),
...
def _page7_recommendations(audit: dict, s: dict) -> list:
    ...
    elements.append(Paragraph(
        ...
        "PakProp AI is not liable for any decisions made based on this report.",
        s['disclaimer'],
    ))
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        f"PakProp AI  |  Property Audit Report  |  Generated {date.today().strftime('%d %B %Y')}",
        s['small'],
    ))
    return elements
...
def generate_audit_pdf(audit_data: dict, output_path: str) -> str:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=MARGIN,
        bottomMargin=MARGIN,
        title='PakProp AI Property Audit Report',
        author='PakProp AI',
    )
